#### OUTLIERS DETECTION vSSG BACKS DATA FRAME STUDY2b ####

#to change the size of a ggplot title:
...
#Loading data:
ssg <- read.csv("./backs_vssg.csv", sep=",")
...
box_tri <- ggplot() + 
  ...
  labs(title = "(g) Stagno's TRIMP")

figure_box_plot <- (box_td | box_hsr | box_pl | box_pls) / 
  (box_acc | box_gu | box_tri) +
  plot_annotation(
    title = "Box plots vSSG backs",
...
sd_tri <- ggplot() +
  ...
  labs(title = "(g) 3*sd: Stagno's TRIMP")
...
figure_sd <- (sd_td | sd_hs | sd_pl | sd_pls) / (sd_acc | sd_gu | sd_tri) +
  plot_annotation(
    title = "Three standard deviations vSSG backs",
    theme = theme(plot.title = element_text(size = 16, hjust = 0.5)))
...
mod_z_scores_hs <- c()
for (i in seq_along(data_hs)) {
  mod_z_scores_hs[i] <- 0.6745*(data_hs[i] - median_hs)/MAD_hs #in this scenario MAD_hs=0
}
mod_z_scores_hs
#Take the absolute value of the modified z-score
...
mod_z_scores_gu <- c()
for (i in seq_along(data_gu)) {
  mod_z_scores_gu[i] <- 0.6745*(data_gu[i] - median_gu)/MAD_gu #in this scenario MAD_gu=0
}
mod_z_scores_gu
#Take the absolute value of the modified z-score
...
figure_mod_z_scores <- (z_td | z_pls | z_pl) /  (z_acc | z_tri) +
  plot_annotation(
    title = "Modified z-scores vSSG backs",
    theme = theme(plot.title = element_text(size = 16, hjust = 0.5)))
...
stu_del_res <- ggplot() + 
  ...
  labs(title = "Studentised delated residuals")
...
r_squared <- ggplot(data = my_r_squared_df) + 
  geom_point(aes(x = as.numeric(row.names(my_r_squared_df)), 
                 y = r_squared)) +
  geom_text_repel(data = subset(my_r_squared_df, r_squared>0.131), 
                  aes(x = as.numeric(
                    row.names(
                      subset(my_r_squared_df, r_squared>0.131))),
                    y = r_squared,
                    label = as.numeric(
                      row.names(
                        subset(my_r_squared_df, r_squared>0.131))
  ...
  xlab("Observation id") +
  ylab("R squared") +
  labs(title = "Model fit outliers: R squared")
...
for(i in seq(1,length(unique(data_train$players)),1)) {
  my_aic_players_list[[i]] <- AIC(
    lmer(stagnos_trimp ~ total_distance_m + 
           ...
           (1|date:ssg_bout) +
           (1|date:players),
         data = subset(data_train, players != unique(data_train$players)[i]), REML = TRUE))
}
...
my_aic_players_df$player_id <- unique(data_train$players)
...
aic_player <- ggplot(data = my_aic_players_df) + 
  geom_point(aes(x = player_id, 
                 y = aic)) +
  geom_text_repel(data = subset(my_aic_players_df, aic<5090), 
                  aes(x = player_id,
                    y = aic,
                    label = player_id), color = "red") +
  xlab("Player id") +
  ylab("AIC") +
  labs(title = "Model fit outliers: AIC for player id")

player_14 <- subset(ssg, players == 14) #one row is zero for trimp
player_29 <- subset(ssg, players == 29) #four rows are zeros for trimp
player_31 <- subset(ssg, players == 31) #one row is zero and seven are in 100s AU for trimp
...
aic_obs <- ggplot(data = my_aic_obs_df) + 
  geom_point(aes(x = as.numeric(row.names(my_aic_obs_df)), 
                 y = aic)) +
  geom_text_repel(data = subset(my_aic_obs_df, aic<5410), 
                  aes(x = as.numeric(
                    row.names(
                      subset(my_aic_obs_df, aic<5410))),
                    y = aic,
                    label = as.numeric(
                      row.names(
                        subset(my_aic_obs_df, aic<5410))
...
dfb_plot_obs_td <- ggplot() +
  ...
  geom_vline(xintercept = 2/sqrt(nrow(dfbetas1)), linetype = 2, size = 0.7) +
  geom_vline(xintercept = -2/sqrt(nrow(dfbetas1)), linetype = 2, size = 0.7) +
  geom_text_repel(data = subset(dfbetas1, total_distance_m > 0.13 | total_distance_m < -0.15),
                  aes(x = total_distance_m,
                      y = as.numeric(
                        row.names(
                          subset(dfbetas1, total_distance_m > 0.13 | total_distance_m < -0.15))),
                      label = as.numeric(
                        row.names(
                          subset(dfbetas1, total_distance_m > 0.13 | total_distance_m < -0.15)))),
                  color = "red") +
...
dfb_plot_obs_hs <- ggplot() +
  ...
  geom_vline(xintercept = 2/sqrt(nrow(dfbetas1)), linetype = 2, size = 0.7) +
  geom_vline(xintercept = -2/sqrt(nrow(dfbetas1)), linetype = 2, size = 0.7) +
  geom_text_repel(data = subset(dfbetas1, tot_hsr_distance > 0.15 | tot_hsr_distance < -0.15),
                  aes(x = tot_hsr_distance,
                      y = as.numeric(
                        row.names(
                          subset(dfbetas1, tot_hsr_distance > 0.15 | tot_hsr_distance < -0.15))),
                      label = as.numeric(
                        row.names(
                          subset(dfbetas1, tot_hsr_distance > 0.15 | tot_hsr_distance < -0.15)))),
                  color = "red") +
...
dfb_plot_obs_pl <- ggplot() +
  ...
  geom_vline(xintercept = 2/sqrt(nrow(dfbetas1)), linetype = 2, size = 0.7) +
  geom_vline(xintercept = -2/sqrt(nrow(dfbetas1)), linetype = 2, size = 0.7) +
  geom_text_repel(data = subset(dfbetas1, total_player_load > 0.15 | total_player_load < -0.15),
                  aes(x = total_player_load,
                      y = as.numeric(
                        row.names(
                          subset(dfbetas1, total_player_load > 0.15 | total_player_load < -0.15))),
                      label = as.numeric(
                        row.names(
                          subset(dfbetas1, total_player_load > 0.15 | total_player_load < -0.15)))),
                  color = "red") +
...
dfb_plot_obs_pls <- ggplot() +
  ...
  geom_vline(xintercept = 2/sqrt(nrow(dfbetas1)), linetype = 2, size = 0.7) +
  geom_vline(xintercept = -2/sqrt(nrow(dfbetas1)), linetype = 2, size = 0.7) +
  geom_text_repel(data = subset(dfbetas1, player_load_slow > 0.15 | player_load_slow < -0.15),
                  aes(x = player_load_slow,
                      y = as.numeric(
                        row.names(
                          subset(dfbetas1, player_load_slow > 0.15 | player_load_slow < -0.15))),
                      label = as.numeric(
                        row.names(
                          subset(dfbetas1, player_load_slow > 0.15 | player_load_slow < -0.15)))),
                  color = "red") +
...
dfb_plot_obs_gu <- ggplot() +
  ...
  geom_vline(xintercept = 2/sqrt(nrow(dfbetas1)), linetype = 2, size = 0.7) +
  geom_vline(xintercept = -2/sqrt(nrow(dfbetas1)), linetype = 2, size = 0.7) +
  geom_text_repel(data = subset(dfbetas1, get_up > 0.15 | get_up < -0.15),
                  aes(x = get_up,
                      y = as.numeric(
                        row.names(
                          subset(dfbetas1, get_up > 0.15 | get_up < -0.15))),
                      label = as.numeric(
                        row.names(
                          subset(dfbetas1, get_up > 0.15 | get_up < -0.15)))),
                  color = "red") +
...
figure_dfbetas <- (dfb_plot_obs_int | dfb_plot_obs_td | dfb_plot_obs_hs |
                     dfb_plot_obs_pl ) / 
  (dfb_plot_obs_pls | dfb_plot_obs_acc | dfb_plot_obs_gu) +
  plot_annotation(title = "DFBETAS general linear model vSSG backs",
                  theme = theme(plot.title = element_text(size = 16, hjust = 0.5)))
...
figure_dfbetas_mixed_players <- (dfb_plot_mixed_players_int | dfb_plot_mixed_players_td | 
  ...
  (dfb_plot_mixed_players_pls | dfb_plot_mixed_players_acc | 
     dfb_plot_mixed_players_gu) +
  plot_annotation(title = "DFBETAS mixed model player ids vSSG backs",
                  theme = theme(plot.title = element_text(size = 16, hjust = 0.5)))
...
figure_dfbetas_mixed_obs <- (dfb_plot_mixed_obs_int | dfb_plot_mixed_obs_td | 
  ...
  (dfb_plot_mixed_obs_pls | dfb_plot_mixed_obs_acc |
     dfb_plot_mixed_obs_gu)

#creating final figure for DFBETAS mixed model observations
figure_dfbetas_mixed_obs + 
  plot_annotation(title = "DFBETAS mixed model observation ids vSSG backs",
...
dffits_plot_linear <- ggplot() + 
  ...
  geom_text_repel(data = subset(dffits0, 
                  ...
                  color = "red") +
  xlab("Observation id") + ylab("DFFITS") +
  labs(title = "DFFITS general linear model vSSG backs")
...
dffits_mixed_players$player_id <- unique(ssg$players)
...
dffits_plot_mixed_players <- ggplot() + 
  ...
  xlab("Player id") + ylab("DFFITS") +
  labs(title = "DFFITS mixed model for player id vSSG backs")
...
dffits_plot_mixed <- ggplot() + 
  ...
  geom_text_repel(data = subset(dffits_mixed, 
                  ...
                  color = "red") +
  xlab("Observation id") + ylab("DFFITS") +
  labs(title = "DFFITS mixed model for observation id vSSG backs")

#Figure for DFFITS
figure_dffits <- dffits_plot_linear + dffits_plot_mixed_players + dffits_plot_mixed +
  plot_layout(width = c(2,1)) +
  plot_annotation(title = "DFFITS general linear (mixed) models for vSSG backs",
                  theme = theme(plot.title = element_text(size = 16, hjust = 0.5)))
...
cooks_plot_linear <- ggplot() + 
  ...
  labs(title = "Cook's distance general linear model vSSG backs")
...
cooks_mixed_players_plot <- ggplot() + 
  ...
  labs(title = "Cook's distance mixed model per player id vSSG backs")
...
cooks_mixed_obs_plot <- ggplot() + 
  ...
  geom_text_repel(data = subset(cooks_d_mixed, d>4/nrow(cooks_d_mixed)), 
                  ...
                  color = "red") +
  xlab("Observation id") + ylab("Cook's d") +
  labs(title = "Cook's distance mixed model per observation vSSG backs")
...
figure_cooks_d <- cooks_plot_linear + cooks_mixed_players_plot + cooks_mixed_obs_plot +
  ...
  plot_annotation(title = "Cook's distance vSSG backs",
                  theme = theme(plot.title = element_text(size = 16, hjust = 0.5)))
...
aic_player_id <- subset(my_aic_players_df, aic<5090)$player_id
...
dffits_mix_pl_rows <- subset(dffits_mixed_players, dffit>0.05)$player_id
...
#split the data frame into two groups (58/2=29) for the final plot
outliers_df_copy$group <- c(rep(1, 29), rep(2, 29))
...
final_outliers_plot <- ggplot() +
  ...
  labs(title = "Observations detected as potential outliers vSSG backs")
...
final_player_outliers_plot <- ggplot(all_player_outliers_df) + 
  ...
  labs(title = "Players detected as potential outliers vSSG backs")
