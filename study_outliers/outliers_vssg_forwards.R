#### OUTLIERS DETECTION VSSG FORWARDS DATA FRAME STUDY2b ####
...
#Loading data:
ssg <- read.csv("./forwards_vssg.csv", sep=",")
...
figure_box_plot <- (box_td | box_hsr | box_pl | box_pls) / 
  ...
  plot_annotation(title = "Box plot vSSG forwards",
...
figure_sd <- (sd_td | sd_hs | sd_pl | sd_pls) / (sd_acc | sd_gu | sd_bu | sd_tri) +
  plot_annotation(title = "3*SD vSSG forwards",
                  theme = theme(plot.title = element_text(size = 16, hjust = 0.5)))
...
figure_mod_z_scores <- (z_td | z_pls | z_pl) /  (z_acc | z_tri) +
  plot_annotation(title = "Modified z-score vSSG forwards",
...
r_squared <- ggplot(data = my_r_squared_df) + 
  geom_point(aes(x = as.numeric(row.names(my_r_squared_df)), 
                 y = r_squared)) +
  geom_text_repel(data = subset(my_r_squared_df, r_squared>0.015), 
                  aes(x = as.numeric(
                    row.names(
                      subset(my_r_squared_df, r_squared>0.015))),
                    y = r_squared,
                    label = as.numeric(
                      row.names(
                        subset(my_r_squared_df, r_squared>0.015))
...
for(i in seq(1,length(unique(data_train$players)),1)) {
  my_aic_players_list[[i]] <- AIC(
    lmer(stagnos_trimp ~ total_distance_m + 
           ...
           (1|date:ssg_bout) +
           (1|date:players),
         data = subset(data_train, players != sort(unique(data_train$players))[i]), REML = TRUE))
}
...
aic_player <- ggplot(data = my_aic_players_df) + 
  geom_point(aes(x = as.numeric(row.names(my_aic_players_df)), 
                 y = aic)) +
  geom_text_repel(data = subset(my_aic_players_df, aic<5900), 
                  aes(x = as.numeric(
                    row.names(
                      subset(my_aic_players_df, aic<5900))),
                    y = aic,
                    label = as.numeric(
                      row.names(
                        subset(my_aic_players_df, aic<5900))
...
player_19 <- subset(ssg, players == 19) #four rows are suspicious trimp values (<187)
...
aic_obs <- ggplot(data = my_aic_obs_df) + 
  geom_point(aes(x = as.numeric(row.names(my_aic_obs_df)), 
                 y = aic)) +
  geom_text_repel(data = subset(my_aic_obs_df, aic<6245), 
                  aes(x = as.numeric(
                    row.names(
                      subset(my_aic_obs_df, aic<6245))),
                    y = aic,
                    label = as.numeric(
                      row.names(
                        subset(my_aic_obs_df, aic<6245))
...
dfb_plot_obs_int <- ggplot() +
  ...
  geom_vline(xintercept = -2/sqrt(nrow(dfbetas1)), linetype = 2, size = 0.7) +
  geom_text_repel(data = subset(dfbetas1, `(Intercept)` > 0.15 | `(Intercept)` < -0.15),
                  aes(x = `(Intercept)`,
                      y = as.numeric(
                        row.names(
                          subset(dfbetas1, `(Intercept)` > 0.15 | `(Intercept)` < -0.15))),
                      label = as.numeric(
                        row.names(
                          subset(dfbetas1, `(Intercept)` > 0.15 | `(Intercept)` < -0.15)))),
                  color = "red") +
...
dfb_plot_obs_td <- ggplot() +
  ...
  geom_vline(xintercept = 2/sqrt(nrow(dfbetas1)), linetype = 2, size = 0.7) +
  geom_vline(xintercept = -2/sqrt(nrow(dfbetas1)), linetype = 2, size = 0.7) +
  geom_text_repel(data = subset(dfbetas1, total_distance_m > 0.15 | total_distance_m < -0.15),
                  aes(x = total_distance_m,
                      y = as.numeric(
                        row.names(
                          subset(dfbetas1, total_distance_m > 0.15 | total_distance_m < -0.15))),
                      label = as.numeric(
                        row.names(
                          subset(dfbetas1, total_distance_m > 0.15 | total_distance_m < -0.15)))),
                  color = "red") +
...
dfb_plot_obs_acc <- ggplot() +
  ...
  geom_vline(xintercept = 2/sqrt(nrow(dfbetas1)), linetype = 2, size = 0.7) +
  geom_vline(xintercept = -2/sqrt(nrow(dfbetas1)), linetype = 2, size = 0.7) +
  geom_text_repel(data = subset(dfbetas1, acceleration_density > 0.15 | acceleration_density < -0.15),
                  aes(x = acceleration_density,
                      y = as.numeric(
                        row.names(
                          subset(dfbetas1, acceleration_density > 0.15 | acceleration_density < -0.15))),
                      label = as.numeric(
                        row.names(
                          subset(dfbetas1, acceleration_density > 0.15 | acceleration_density < -0.15)))),
                  color = "red") +
...
dfb_plot_obs_bu <- ggplot() +
  ...
  geom_vline(xintercept = 2/sqrt(nrow(dfbetas1)), linetype = 2, size = 0.7) +
  geom_vline(xintercept = -2/sqrt(nrow(dfbetas1)), linetype = 2, size = 0.7) +
  geom_text_repel(data = subset(dfbetas1, bullet > 0.15 | bullet < -0.15),
                  aes(x = bullet,
                      y = as.numeric(
                        row.names(
                          subset(dfbetas1, bullet > 0.15 | bullet < -0.15))),
                      label = as.numeric(
                        row.names(
                          subset(dfbetas1, bullet > 0.15 | bullet < -0.15)))),
                  color = "red") +
...
figure_dfbetas <- (dfb_plot_obs_int | dfb_plot_obs_td | dfb_plot_obs_hs |
                     dfb_plot_obs_pl ) / 
  (dfb_plot_obs_pls | dfb_plot_obs_acc | dfb_plot_obs_gu | dfb_plot_obs_bu) +
  plot_annotation(title = "DFBETAS general linear model vSSG forwards",
                  theme = theme(plot.title = element_text(size = 16, hjust = 0.5)))
...
figure_dfbetas_mixed_players <- (dfb_plot_mixed_players_int | dfb_plot_mixed_players_td | 
  ...
  plot_annotation(title = "DFBETAS mixed model player id vSSG forwards",
...
figure_dffits <- dffits_plot_linear + dffits_plot_mixed_players + dffits_plot_mixed +
  ...
  plot_annotation(title = "DFFITS vSSG forwards",
                  theme = theme(plot.title = element_text(size = 16,
                                                          hjust = 0.5)))
...
figure_cooks_d <- cooks_plot_linear + cooks_mixed_players_plot + cooks_mixed_obs_plot +
  ...
  plot_annotation(title = "Cook's distance vSSG forwards",
                  theme = theme(plot.title = element_text(
                    size = 16, hjust = 0.5
                  )))
...
#split the data frame into three groups (67/3=22+22+23) for the final plot
outliers_df_copy$group <- c(rep(1, 22), rep(2, 22), rep(3, 23))
...
final_outliers_plot <- ggplot() +
  ...
  labs(title = "Observations detected as potential outliers vSSG forwards")
...
final_player_outliers_plot <- ggplot(all_player_outliers_df) + 
  ...
  labs(title = "Players detected as potential outliers vSSG forwards")
