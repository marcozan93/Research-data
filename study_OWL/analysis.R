...
#### Drop out investigation ####
a <- nrow(subset(data, group == 0 & time == 0))
b <- nrow(subset(data, group == 1 & time == 0))
c <- nrow(subset(data, group == 0 & time == 1))
d <- nrow(subset(data, group == 1 & time == 1))

(c - a) / a # control % dropout = 43.48%

(d - b) / b # treatment % dropout = 26.09%
...
#control=0  | a-c      |.  c          | 
#treatme=1  | b-d      |.  d.         | 
control_dropouts <- (a - c)
treatment_dropouts <- (b - d)
control_remaining <- c
treatment_remaining <- d
fisher.test(rbind(c(control_dropouts,control_remaining), 
                  c(treatment_dropouts,treatment_remaining)), 
            alternative="two.sided"
            )
# COMMENT: There is no evidence to support the existence of differences in the   (!)  
# dropout patterns for both groups (Fisher exact test: p = 0.3534).
...
bxp_5rm <- create_custom_boxplot(data_with_outliers, "X5rm", "5RM back squat (kg)", 
                                 "Outlier_5rm")
...
aic_player <- ggplot(data = my_aic_players_df) + 
  geom_point(aes(x = as.numeric(row.names(my_aic_players_df))-1, 
                 y = aic)) +
  geom_text_repel(data = subset(my_aic_players_df, aic< -97), 
                  aes(x = as.numeric(
                    row.names(
                      subset(my_aic_players_df, aic< -97)))-1,
                    y = aic,
                    label = as.numeric(
                      row.names(
                        subset(my_aic_players_df, aic< -97))
                    )-1), color = "red") +
  xlab("Player id") +
...
aic_obs <- ggplot(data = my_aic_obs_df) + 
  geom_point(aes(x = as.numeric(row.names(my_aic_obs_df)), 
                 y = aic)) +
  geom_text_repel(data = subset(my_aic_obs_df, aic< -98), 
                  aes(x = as.numeric(
                    row.names(
                      subset(my_aic_obs_df, aic< -98))),
                    y = aic,
                    label = as.numeric(
                      row.names(
                        subset(my_aic_obs_df, aic< -98))
...
#### CMJ ####
cmj_anova <- aov(cmj ~ time*group, data)
summary(cmj_anova)
...
anova(cmj_model1, cmj_anova)
# COMMENT: Evidence to support the mixed-effect model against anova p = 6.253e-14
...
# eff_size(emm_cmj, sigma = sigma(cmj_model1), edf = df.residual(cmj_model1))
eff_size(pairs(emm_cmj, simple="group"), sigma(cmj_model1), df.residual(cmj_model1), method='identity')
eff_size(pairs(emm_cmj, simple="time"), sigma(cmj_model1), df.residual(cmj_model1), method='identity')
...
############################## X10m ####################################
data_no_outliers_10m <- data[-c(1, 23, 24),]

X10m_anova <- aov(X10m ~ time*group, data)
summary(X10m_anova)
...
anova(X10m_model1, X10m_anova)
# COMMENT: Evidence to support X10m_model1 against anova p = 5.056e-09
...
eff_size(pairs(X10m_emm, simple="group"), sigma(X10m_model2), df.residual(X10m_model2), method='identity')
eff_size(pairs(X10m_emm, simple="time"), sigma(X10m_model2), df.residual(X10m_model2), method='identity')
...
# OK: No outliers detected.
# - Based on the following methods and thresholds: zscore_robust (3.291), iqr (2), ci (1), cook (0.9), mahalanobis (22.458), mahalanobis_robust (22.458), mcd (22.458), ics (0.001), optics (12), lof (0.001).
# - For variable: (Whole model)
...
############################## X30m ####################################
X30m_anova <- aov(X30m ~ time*group, data)
summary(X30m_anova)
...
# There is evidence to support model1
anova(X30m_model1, X30m_anova)
# COMMENT: Evidenc to support X30m_model1 against anova p = 3.134e-15
...
eff_size(pairs(X30m_emm, simple="group"), sigma(X30m_model2), df.residual(X30m_model2), method='identity')
eff_size(pairs(X30m_emm, simple="time"), sigma(X30m_model2), df.residual(X30m_model2), method='identity')
...
############################## X5rm ####################################
X5rm_anova <- aov(X5rm ~ time*group, data)
summary(X5rm_anova)
...
X5rm_model2 <- lmer(X5rm ~ time*group + weight_scaled +
                      (1 | id), data = data)

anova(X5rm_model, X5rm_model1)
# There is evidence to support model1 (p = 0.04753)
anova(X5rm_model1, X5rm_anova)
# COMMENT: Evidence to support X5rm_model1 against anova p = 7.375e-15

anova(X5rm_model2, X5rm_model)
anova(X5rm_model2, X5rm_model1)

sjPlot::tab_model(X5rm_model2)


# Creating Estimated Marginal Means
X5rm_emm <- emmeans(X5rm_model2, ~ group*time)
# Pairwise comparisons for time within each group
...
eff_size(pairs(X5rm_emm, simple="group"), sigma(X5rm_model2), df.residual(X5rm_model2), method='identity')
eff_size(pairs(X5rm_emm, simple="time"), sigma(X5rm_model2), df.residual(X5rm_model2), method='identity')



# return a list of single plots
diagnostic_plots_5rm <- plot(check_model(X5rm_model2, panel = FALSE))
...
check_heteroscedasticity(X5rm_model2)
# Warning: Heteroscedasticity (non-constant error variance) detected (p = 0.032).

# influential observations - outliers
check_outliers(X5rm_model2)
# OK: No outliers detected.
# - Based on the following method and threshold: cook (0.9).
# - For variable: (Whole model)
...
check_collinearity(X5rm_model2) # All good here.
...
# COMMENT: All assumptions are satisfied apart from heteroscedasticity.                                         (8)

# This should be my final model parameters !!
model_parameters(
  X5rm_model2,
  vcov = "vcovCR",
  vcov_args = list(type = "CR1", cluster = data$id)
)

## 5RM
pred_5rm <- ggpredict(X5rm_model1, terms = c("time", "group"))
